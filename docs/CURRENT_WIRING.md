...
The two 18650 cells feed a BMS. The BMS output feeds the servo driver through a switch and also feeds a 5 V BEC for the Pi and other components. The BMS and BEC current ratings have not been confirmed. The builder found that the earlier cells could not supply enough current during walking and the Pi shut down from undervoltage. A fully charged two-cell Molicel P28A pack measured 8.4 V at the pack and completed nearly one minute of walking without a Pi shutdown. The voltage reaching the servos with this pack has not been measured.
...
5. All 14 motors followed a supported, 20-second move into the walk start pose at gain 8. While holding that pose, leg gains were raised in stages to 30 and head gains stayed at 8. The largest position error fell to about 0.03 rad, the Pi stayed connected, and `vcgencmd get_throttled` stayed at `0x0`. The duck returned to its starting pose. That pose check did not test dynamic walking. `scripts/check_motors.py` changes gains and includes movement tests, so it is not a read-only diagnostic.
6. The Bluetooth controller paired and Linux received its buttons and sticks. Pygame's controller class detected A presses over SSH once background joystick input was enabled. The first policy run reached the starting pose, then the motor serial connection failed shortly after unpausing. The Pi stayed on, but the walk did not complete. A later brief, supported run moved the legs without strain or another serial error. Its kernel log had no USB error, `/dev/ttyACM0` stayed present, and the Pi reported `throttled=0x0`.
7. The motor bus completed 250 position and velocity reads at 50 Hz without errors. It also completed 250 read and goal-write cycles at 50 Hz. Although torque was off before the goal-write test, all 14 torque flags read enabled afterward. Torque was then disabled and verified off. Treat manual goal writes as commands that can turn torque on.
8. In a longer supported run, the legs stopped suddenly after about 30 seconds of policy motion and SSH timed out. Ctrl+C did not reach the Pi, so the shared supply was switched off. The power monitor's last complete entry, at 14:03:17, still showed `throttled=0x0` and `/dev/ttyACM0` present; it did not record the instant the legs stopped. The kernel follow log was empty. After reboot, all 14 torque flags were off and `git fsck --full` passed.
9. On 24 September, a fully charged Molicel P28A pack let the duck walk for almost one minute without the Pi shutting down. This supports the builder's diagnosis that the earlier cells browned out under walking load. This run does not establish whether the separate motor feedback timeouts are resolved.

The old-pack Pi shutdowns are consistent with the battery current shortfall. The cause of motor feedback timeouts while the Pi stayed on remains unconfirmed. Logs from the first failure were lost when the shared supply was switched off. The walk code now stops when motor feedback is lost and warns if it cannot turn off torque over the failed connection. In that case, support the duck and switch off the shared power supply. The earlier SSH disconnect during `find_soft_offsets.py` has not been explained either. That script uses default offsets and raised all 14 motor gains to 32, unlike the supported test above.
